...
public class CleanInteractionsCommand extends AbstractPlayerCommand {
    ...
    public CleanInteractionsCommand(HyzenKernel plugin) {
        super("cleaninteractions", "hyzenkernel.command.cleaninteractions.desc");
        this.plugin = plugin;
    ...
    protected void execute(
        ...
        if (initFailed) {
            sendMessage(player, "&c[HyzenKernel] CleanInteractions API not available - see server logs");
            return;
        }
    ...
    private void exploreInteractionModule(Player player, World world) {
        sendMessage(player, "&6[HyzenKernel] Exploring InteractionModule structure...");
        ...
        try {
            ...
            sendMessage(player, "&c[HyzenKernel] Error exploring: " + e.getMessage());
    ...
    private void exploreInteractionZones(Player player, World world) {
        sendMessage(player, "&6[HyzenKernel] Exploring interaction zones near you...");
        plugin.getLogger().at(Level.INFO).log("[CleanInteractions] ========================================");
    ...
    private void checkBlockInteraction(Player player, World world, int x, int y, int z) {
        sendMessage(player, "&6[HyzenKernel] Checking block interaction at " + x + ", " + y + ", " + z);
        plugin.getLogger().at(Level.INFO).log("[CleanInteractions] ========================================");
    ...
    private void scanForOrphans(Player player, Store<EntityStore> store, Ref<EntityStore> playerRef, World world, boolean remove) {
        try {
            sendMessage(player, "&6[HyzenKernel] Scanning for orphaned interactions...");
            ...
            sendMessage(player, "&c[HyzenKernel] Error during scan: " + e.getMessage());
    ...
    private void exploreTrackedPlacement(Player player, World world, int x, int y, int z, boolean clean) {
        sendMessage(player, "&6[HyzenKernel] Exploring TrackedPlacement at " + x + ", " + y + ", " + z);
        plugin.getLogger().at(Level.INFO).log("[CleanInteractions] ========================================");
    ...
    private void findAllEntitiesNear(Player player, World world, double tx, double ty, double tz, double radius) {
        sendMessage(player, "&6[HyzenKernel] Finding ALL entities near " + tx + ", " + ty + ", " + tz + " (radius " + radius + ")");
        plugin.getLogger().at(Level.INFO).log("[CleanInteractions] ========================================");
