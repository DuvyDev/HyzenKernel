...
public class ChunkProtectionCommand extends AbstractPlayerCommand {
    ...
    public ChunkProtectionCommand(HyzenKernel plugin) {
        super("chunkprotect", "hyzenkernel.command.chunkprotect.desc");
        this.plugin = plugin;
    ...
    protected void execute(
        ...
        if (registry == null) {
            sendMessage(player, "&c[HyzenKernel] Chunk protection system is not enabled");
    ...
    private void forceScan(Player player, World world, ChunkProtectionRegistry registry, ChunkProtectionScanner scanner) {
        if (scanner == null) {
            sendMessage(player, "&c[HyzenKernel] Scanner not available");
            return;
        }

        sendMessage(player, "&6[HyzenKernel] Starting chunk protection scan...");
        ...
        try {
            ...
            sendMessage(player, "&a[HyzenKernel] Scan complete!");
            ...
            sendMessage(player, "&c[HyzenKernel] Scan error: " + e.getMessage());
        }
    }
    ...
    private void clearAllProtection(Player player, ChunkProtectionRegistry registry) {
        ...
        if (count == 0) {
            sendMessage(player, "&7[HyzenKernel] No chunks are currently protected.");
            return;
        }
        
        sendMessage(player, "&c[HyzenKernel] WARNING: Clearing all chunk protection!");
        sendMessage(player, "&c[HyzenKernel] This may allow teleporters to be cleaned up!");
        
        int cleared = registry.clearAllProtections();
        
        sendMessage(player, "&6[HyzenKernel] Cleared protection from " + cleared + " chunks.");
    ...
    private void checkCurrentChunk(Player player, Store<EntityStore> store, Ref<EntityStore> ref,
        ...
        try {
            ...
            if (transform == null) {
                sendMessage(player, "&c[HyzenKernel] Cannot get your position");
                return;
            }
            ...
            sendMessage(player, "&c[HyzenKernel] Error: " + e.getMessage());
        }
    }
    ...
    private void debugNearbyEntities(Player player, Store<EntityStore> store, Ref<EntityStore> ref,
        ...
        try {
            ...
            if (entityStore == null) {
                sendMessage(player, "&c[HyzenKernel] Cannot access entity store");
                return;
            }
            ...
            if (forEachChunkMethod == null) {
                sendMessage(player, "&c[HyzenKernel] Cannot find forEachChunk method");
                return;
            }
    ...
    private void debugTeleporters(Player player, World world) {
        ...
        try {
            ...
            if (chunkStore == null) {
                sendMessage(player, "&c[HyzenKernel] Cannot access ChunkStore");
                return;
            }

            Store<ChunkStore> store = chunkStore.getStore();
            if (store == null) {
                sendMessage(player, "&c[HyzenKernel] Cannot access ChunkStore's Store");
                return;
            }
            ...
            try {
                ...
                sendMessage(player, "&c[HyzenKernel] TeleporterPlugin not found!");
            ...
            if (teleporterPlugin == null) {
                sendMessage(player, "&c[HyzenKernel] TeleporterPlugin.get() returned null");
                return;
            }
            ...
            if (teleporterComponentType == null) {
                sendMessage(player, "&c[HyzenKernel] getTeleporterComponentType() returned null");
                return;
            }
            ...
            if (forEachChunkMethod == null) {
                sendMessage(player, "&c[HyzenKernel] Cannot find forEachChunk(Query, BiConsumer) method");
                return;
            }
