...
/**
 * HyzenKernel Admin Dashboard - Visual UI for managing and monitoring HyzenKernel.
 *
 * Uses CommandListPage.ui as the base layout with BasicTextButton.ui for tabs
 * and SubcommandCard.ui for content display.
 */
public class HyzenKernelDashboardPage extends InteractiveCustomUIPage<DashboardEventData> {
    ...
    public HyzenKernelDashboardPage(PlayerRef playerRef) {
    ...
    public void build(Ref<EntityStore> ref, UICommandBuilder cmd, UIEventBuilder events, Store<EntityStore> store) {
        try {
            ...
            // Set the title
            cmd.set("#CommandName.Text", "HyzenKernel Dashboard");
            ...
            HyzenKernel.getInstance().getLogger().at(Level.WARNING).withCause(e)
    ...
    private int buildOverviewPluginList(UICommandBuilder cmd, UIEventBuilder events, int index) {
        ...
        index = addPluginItem(cmd, index, "Crashes Prevented", String.valueOf(stats.getTotalCrashesPrevented()), "STAT", "Total server crashes prevented by HyzenKernel");
    ...
    private int buildConfigPluginList(UICommandBuilder cmd, UIEventBuilder events, int index) {
        ...
        HyzenKernelConfig config = configManager.getConfig();
        ...
        index = addPluginItem(cmd, index, "Config File", loadedStatus, "INFO", "plugins/hyzenkernel/config.json");
    ...
    private int buildOverviewContent(UICommandBuilder cmd, UIEventBuilder events, int index) {
        ...
        index = addContentCard(cmd, events, index,
                ...
                "Total server crashes prevented by HyzenKernel sanitizers", null);
        ...
        index = addContentCard(cmd, events, index,
                ...
                "Current HyzenKernel plugin version", null);
    ...
    private int buildConfigContent(UICommandBuilder cmd, UIEventBuilder events, int index) {
        ...
        String loadedStatus = configManager.isLoadedFromFile() ? "YES" : "NO (defaults)";
        index = addContentCard(cmd, events, index,
                "Config: plugins/hyzenkernel/config.json",
    ...
    private void notifyPlayer(Ref<EntityStore> ref, Store<EntityStore> store, String message) {
        ...
        if (player != null) {
            ChatColorUtil.sendMessage(player, "&a[HyzenKernel] " + message);
        }
    }
}
